...
class ParseConfig {
  ...
  static save(attrs) {
    const controller = CoreManager.getConfigController(); //To avoid a mismatch with the local and the cloud config we get a new version
  ...
  save(attrs) {
    ...
    return RESTController.request('PUT', 'config', {
      ...
      if (response && response.result) {
        ...
        const error = new ParseError(ParseError.INTERNAL_SERVER_ERROR, 'Error occured updating Config.');
        return Promise.reject(error);
      }
    });
  }
